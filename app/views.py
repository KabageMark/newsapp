...
def sources():
    ...
    return render_template('sources.html', title = title,general = source_news_general,Url = source_news_url,sports = source_news_sports,technology = source_news_technology,business = source_news_business) #image = image_news, description = description_news url=url_news)


@app.route('/sources/articles/<id>')
def articles(id):
    articles_args = get_articles(id)
    return render_template('articles.html',articles = articles_args ) #image = image_news, description = description_news url=url_news)
